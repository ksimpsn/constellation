...
from backend.core.database import (
    save_job, update_job, get_job, get_all_jobs, delete_job, SessionLocal, Job, init_db, Base, engine
)
...
class TestJobStatusPersistence(unittest.TestCase):
    ...
    def test_update_job_modifies_fields(self):
        ...
        success = update_job(job_id, status="complete", results=[1, 4, 9])
        self.assertTrue(success)
        
        job = get_job(job_id)
        self.assertEqual(job.status, "complete")
        self.assertEqual(job.results, [1, 4, 9])
