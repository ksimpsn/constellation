        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 items-stretch w-full">
          ...
          <div className="lg:w-[280px] shrink-0 flex flex-col">
            <div className="p-6 sm:p-8 rounded-2xl bg-white/[0.06] backdrop-blur-md border border-white/10 flex flex-col items-center text-center">
              ...
              <span className="inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 rounded-full bg-violet-500/20 border border-violet-400/30 text-violet-200 text-xs font-medium">
                {roleLabel}
              </span>
            </div>
            {/* Quick links */}
          ...
          {/* Right: Stats + account info */}
          <div className="flex-1 min-w-0 flex flex-col gap-6">
            {/* Stats row */}
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              {[
                { value: loading ? '...' : String(stats.totalProjects), label: 'Projects', accent: 'violet' },
                { value: loading ? '...' : String(stats.completedProjects), label: 'Completed', accent: 'emerald' },
                { value: loading ? '...' : String(stats.totalContributors), label: 'Contributors', accent: 'violet' },
