...
/** Static projects — no API / backend. */
const HARDCODED_PROJECTS: ResearchProject[] = [
  {
    id: "demo-4",
    title: "text processing",
    description: "Text processing project.",
    progress: 100,
    totalContributors: 2,
    completedContributors: 2,
...
export default function ResearcherDashboard() {
  ...
  useEffect(() => {
    // Keep a small "loading" moment for UI parity with the real dashboard.
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full items-start">
            {projects.map((proj) => {
              ...
              const totalTasks = proj.totalTasks;
              const failedTasks = proj.failedTasks;

              const completedTasks = proj.completedTasks;
                    ...
                    <div className="mt-3 p-3 rounded-xl bg-white/5 border border-white/10 text-sm">
                      ...
                      <div className="flex justify-between mb-1.5">
                        ...
                        <span className="font-semibold text-amber-400/90">
                          {(totalTasks - completedTasks - failedTasks).toLocaleString()}
                        </span>
                      </div>
                      {failedTasks > 0 && (
                        <div className="flex justify-between">
                          <span className="text-white/60">Tasks Failed:</span>
                          <span className="font-semibold text-red-400/90">{failedTasks}</span>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="mt-5 pt-4 border-t border-white/10 animate-[fadeIn_0.3s_ease-in]">
                      ...
                      <div className="mb-4 p-3 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <div className="text-sm">
                          ...
                          <p className="text-[11px] text-white/50 m-0 mb-1.5 pl-1">
                            Tasks waiting to be processed or currently being worked on by volunteers
                          </p>
                          {failedTasks > 0 && (
                            <>
                              <div className="flex justify-between mb-1.5">
                              ...
                              <p className="text-[11px] text-white/50 m-0 mb-1.5 pl-1">
                                Tasks that encountered errors and may be retried automatically
                              </p>
                            </>
                          )}
